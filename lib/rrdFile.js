 ...
 * @constructor
 */
function RRDRRAInfo(rrd_data, rra_def_idx, int_width, nrRows, pdp_step, my_idx) {
	this.nrRows = nrRows;
	/** @type {Number} which RRA it is in the RRD file.  */
	this.my_idx = my_idx;
 ...
 * @constructor
 */
function RRDRRA(rrd_data, rra_ptr_idx, rra_info, header_size, prev_row_cnts, ds_cnt) {
	this.rrd_data = rrd_data;
	this.rra_info = rra_info;
	/** @return {Number} the number of rows in the RRA.  */
	this.nrRows = rra_info.nrRows;
	...
	this.data = new Array(ds_cnt).fill(0).map(row => new Array(this.row_size).fill(0));

	for (var y = 0; y < ds_cnt; y++) {
		for (var x = 0; x < this.nrRows; x++) {
			// it is round robin, starting from cur_row+1
			this.data[y][x] = this.rrd_data.getDoubleAt(this.base_rrd_db_idx + this.row_size * ((x + this.cur_row + 1) % this.nrRows) + y * 8);
		}
	}
}
...
RRDFile.prototype = {
	...
	calc_idxs : function() {
		...
		this.row_cnt_idx = Math.ceil(20 / this.int_width) * this.int_width;
	...
	getDSNames : function() {
		...
		for (var idx = 0; idx < this.ds_cnt; idx++) {
			ds_names.push(this.getDSbyIdx(idx).name);
		}
		return ds_names;
	},
	...
	getRRAInfo : function(idx) {
		...
		return new RRDRRAInfo(this.rrd_data,
				...
				this.int_width, this.rra_def_row_cnts[idx], this.pdp_step,
				idx);
	},
	 ...
	 */
	getRRA : function(idx) {
		return new RRDRRA(this.rrd_data,
				this.rra_ptr_idx + idx * this.rra_ptr_el_size,
				this.getRRAInfo(idx),
				this.header_size,
				this.rra_def_row_cnt_sums[idx],
				this.ds_cnt);
	}
};
