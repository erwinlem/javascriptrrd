...
function RRDRRA(rrd_data, rra_ptr_idx, rra_info, header_size, prev_row_cnts, ds_cnt) {
	...
	this.data = new Array(ds_cnt).fill(0).map(row => new Array(rra_info.nrRows));

	for (var y = 0; y < ds_cnt; y++) {
		for (var x = 0; x < rra_info.nrRows; x++) {
			// it is round robin, starting from cur_row+1
			this.data[y][x] = rrd_data.getDoubleAt(this.base_rrd_db_idx + (ds_cnt * ((x + this.cur_row + 1) % rra_info.nrRows) + y) * 8);
		}
	}
}
