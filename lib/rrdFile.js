...
function RRDDS(rrd_data, rrd_data_idx, my_idx) {
	...
	this.name = this.rrd_data.getCStringAt(this.rrd_data_idx, 20);
	/** @type {string} the type of the data source.  */
	this.type = this.rrd_data.getCStringAt(this.rrd_data_idx + 20, 20);
	/** @type {Number} the minimum value the data source can contain.  */
	this.min = this.rrd_data.getDoubleAt(this.rrd_data_idx + 48);
	/** @type {Number} the maximum value the data source can contain.  */
	this.max = this.rrd_data.getDoubleAt(this.rrd_data_idx + 56);
}
...
function RRDRRAInfo(rrd_data, rra_def_idx,
	int_align, nrRows, pdp_step, my_idx) {
	this.rrd_data = rrd_data;
