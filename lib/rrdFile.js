...
RRDRRA.prototype = {
	...
	loadData : function(rrd_data, rra_ptr_idx, header_size, prev_row_cnts, ds_cnt, nrRows, rra_def_idx, int_width, pdp_step) {
		this.nrRows = nrRows;

		/** @type {Number} the number of seconds between rows.  */
		this.step = pdp_step * this.pdp_cnt;
...
function RRDFile(bf) {
	...
	this.rrd_data.readNop(this.int_width*2);
	...
	this.load_row_cnts();
	for (i = 0; i < this.rra_cnt; i ++) {
		this.rra[i].loadData(this.rrd_data,
