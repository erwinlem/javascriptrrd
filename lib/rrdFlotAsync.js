...
rrdFlotMatrixAsync.prototype.callback = function() {
	if (this.rrd_flot_obj !== null) {
		delete this.rrd_flot_obj;
	}
	...
	for (var i in this.loaded_data) {
		...
		var el = this.loaded_data[i];
		if (el !== undefined) {
			real_data_arr.push([this.url_pair_list[i][0], el]);
		}
	}
	this.rrd_data = real_data_arr[0];

	if (this.customization_callback !== undefined) {
		this.customization_callback(this);
	}

	// apply RRDFilterOp
	if (this.ds_op_list !== undefined) {
		for (i in real_data_arr) {
			real_data_arr[i][1] = new RRDFilterOp(real_data_arr[i][1], this.ds_op_list);
		}
	}

	// apply RRDRRAFilterAvg
	if (this.rra_op_list !== undefined && this.rra_op_list !== null) {
		for (i in real_data_arr) {
			real_data_arr[i][1] = new RRDRRAFilterAvg(real_data_arr[i][1], this.rra_op_list);
		}
	}
