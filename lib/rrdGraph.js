...
rrdGraph.prototype.getFlotData = function() {
	...
	$.each(rrd.rrd_header.getDSNames(), function (key,value) {
		var data = [];
		var x,y;
		var ds = rrd.rrd_header.getDS(value);	

		// calculate start, end and resolution for each rra
		var rraRange = [];
		for (y = 0; y < rrd.rrd_header.getNrRRAs(); y++) { 
			var rra = rrd.getRRA(y);
			rraRange[y] = [
				(rrd.rrd_header.getLastUpdate() - (rra.nrRows-1)* rra.rra_info.step)*1000.,
				rrd.rrd_header.getLastUpdate() * 1000.,
				rra.rra_info.step
			];
		}
		...
		for (y = 0; y < rrd.rrd_header.getNrRRAs(); y++) { 
			for (x = 0; x < rrd.rrd_header.getNrRRAs(); x++) {
		...
		for (y = 0; y < rrd.rrd_header.getNrRRAs(); y++) { 
			...
			for (x = rra.nrRows-1; x > 0; x--) {
				...
				var time = (rrd.rrd_header.getLastUpdate() - (rra.nrRows-1-x) * rra.rra_info.step) * 1000.;
